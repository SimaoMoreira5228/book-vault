...
async fn raw_source_returns_source_file() {
	...
	if !book_id.is_empty() {
		...
		assert!(
			content_type.contains("epub") || content_type.contains("octet-stream"),
...
async fn create_book_empty_title() {
	...
	assert!(
		...
		"should accept or reject empty title: {status}"
	);
}
...
async fn duplicate_annotation_same_position() {
	...
	assert!(
		ann2.get("id").is_some(),
		"second annotation at same position should also succeed"
	);
...
async fn progress_updates_read_status() {
	...
	assert_eq!(status, 200);
	assert_eq!(
		finished["read_status"], "finished",
		"100% progress should set book to finished"
	);
}
