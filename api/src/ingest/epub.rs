...
fn parse_opf(
        ...
        match reader.read_event_into(&mut buf) {
            ...
            Ok(Event::Empty(ref e)) => {
                ...
                if in_manifest && name == "item" {
                    ...
                    let href = e.attributes().flatten()
                        .find(|a| a.key.as_ref() == b"href")
                        .and_then(|a| String::from_utf8(a.value.to_vec()).ok());
                    let mt = e.attributes().flatten()
                ...
                if in_spine && name == "itemref" {
                    if let Some(idref) = e.attributes().flatten()
    ...
    let spine: Vec<String> = spine_ids.iter()
        .filter_map(|id| {
            manifest.iter()
                .find(|(m_id, _)| m_id == id)
...
fn xhtml_to_blocks(content: &str) -> Result<Vec<Block>, AppError> {
    ...
    for element in document.select(&selector) {
        ...
        let block = match tag {
            ...
            "h1" => Block::Heading { level: 1, spans: extract_spans(&element) },
            "h2" => Block::Heading { level: 2, spans: extract_spans(&element) },
            "h3" => Block::Heading { level: 3, spans: extract_spans(&element) },
            "h4" => Block::Heading { level: 4, spans: extract_spans(&element) },
            "h5" => Block::Heading { level: 5, spans: extract_spans(&element) },
            "h6" => Block::Heading { level: 6, spans: extract_spans(&element) },
