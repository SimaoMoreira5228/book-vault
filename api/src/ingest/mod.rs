...
pub fn decompress(data: &[u8]) -> Result<Vec<u8>, crate::AppError> {
    zstd::decode_all(data)
        .map_err(|e| crate::AppError::Internal(format!("Failed to decompress: {}", e)))
}

pub fn serialize_ir(ir: &BookIr) -> Result<Vec<u8>, crate::AppError> {
    let payload = rmp_serde::to_vec(ir)
        .map_err(|e| crate::AppError::Internal(format!("Failed to serialize IR: {}", e)))?;
    Ok(compress(&payload))
}
