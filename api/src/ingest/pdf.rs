...
async fn parse_pdf(state: &crate::SharedState, book: &books::Model, data: &[u8]) -> Result<BookIr, AppError> {
	let dir = tempfile::tempdir().map_err(|e| AppError::Internal(format!("Temp dir failed: {e}")))?;
	let pdf_path = dir.path().join("input.pdf");
	std::fs::write(&pdf_path, data).map_err(|e| AppError::Internal(format!("Temp write failed: {e}")))?;

	let text =
		pdf_extract::extract_text(&pdf_path).map_err(|e| AppError::Internal(format!("PDF extraction failed: {e}")))?;

	let doc =
		lopdf::Document::load_mem(data).map_err(|e| AppError::Internal(format!("Failed to load PDF with lopdf: {e}")))?;
	...
	for (idx, (page_num, _object_id)) in pages.iter().enumerate() {
			...
			if !images.is_empty() && !blocks.is_empty() {
				...
				for (i, (image_data, mime_type)) in images.iter().enumerate() {
					...
					.await
					{
						...
						Err(e) => {
							tracing::warn!("Failed to store page {} image: {}", idx + 1, e);
						}
					}
				}
		...
		if !blocks.is_empty() {
			let title = if idx == 0 { book.title.clone().into() } else { None };
			spine.push(Section {
				id: Uuid::now_v7(),
				title,
...
fn extract_page_images(doc: &lopdf::Document, page_num: u32) -> Result<Vec<(Vec<u8>, String)>, AppError> {
	...
	for (_name, obj) in xobject_dict.iter() {
		...
		let is_image = stream.dict.get(b"Subtype").ok().and_then(|subtype| subtype.as_name().ok()) == Some(b"Image");
