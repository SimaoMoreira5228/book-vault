...
pub async fn ingest(state: &crate::SharedState, job: &job_queue::Model) -> Result<(), AppError> {
	...
	let dir = tempdir().map_err(|e| AppError::Internal(format!("Temp dir failed: {e}")))?;
	let input_path = dir.path().join("input.mobi");
	let output_path = dir.path().join("output.epub");

	std::fs::write(&input_path, &raw_bytes).map_err(|e| AppError::Internal(format!("Temp write failed: {e}")))?;

	let calibre_result = Command::new("ebook-convert").arg(&input_path).arg(&output_path).output();
