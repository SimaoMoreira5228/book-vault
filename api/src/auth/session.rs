...
impl SessionManager {
	...
	pub async fn create_session(
		...
		let session = sessions::Entity::insert(model).exec_with_returning(db).await?;
		Ok((token, session))
	}

	pub async fn validate_session(db: &sea_orm::DatabaseConnection, token: &str) -> Result<sessions::Model, AppError> {
	...
	pub async fn revoke_session(db: &sea_orm::DatabaseConnection, session_id: Uuid) -> Result<(), AppError> {
		let now: chrono::DateTime<chrono::FixedOffset> = chrono::Utc::now().into();
		let session = Sessions::find_by_id(session_id)
			.one(db)
			.await?
	...
	pub async fn revoke_all_user_sessions(db: &sea_orm::DatabaseConnection, user_id: Uuid) -> Result<(), AppError> {
		let now: chrono::DateTime<chrono::FixedOffset> = chrono::Utc::now().into();
	...
	pub async fn cleanup_expired(db: &sea_orm::DatabaseConnection, retention_days: i64) -> Result<u64, AppError> {
		...
		let result = Sessions::delete_many()
			.filter(
				sessions::Column::ExpiresAt.lt(cutoff).or(sessions::Column::RevokedAt
					.is_not_null()
	...
	pub fn verify_password(hash: &str, password: &str) -> Result<(), AppError> {
		...
		let stored = PasswordHash::new(hash).map_err(|e| AppError::Internal(format!("Invalid password hash: {}", e)))?;
		argon2
