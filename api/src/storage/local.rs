...
impl StorageProvider for LocalFsProvider {
    async fn put(&self, key: &str, data: &[u8]) -> Result<(), crate::AppError> {
        ...
        if let Some(parent) = path.parent() {
            fs::create_dir_all(parent).await.map_err(|e| {
                crate::AppError::Internal(format!("Failed to create directory: {}", e))
            })?;
        }
        fs::write(&path, data).await.map_err(|e| {
            crate::AppError::Internal(format!("Failed to write file: {}", e))
        })?;
        Ok(())
    }
    ...
    async fn get(&self, key: &str) -> Result<Vec<u8>, crate::AppError> {
        ...
        fs::read(&path).await.map_err(|e| {
            crate::AppError::NotFound(format!("Storage file not found: {}", e))
        })
    }
    ...
    async fn delete(&self, key: &str) -> Result<(), crate::AppError> {
        ...
        fs::remove_file(&path).await.map_err(|e| {
            crate::AppError::Internal(format!("Failed to delete file: {}", e))
        })?;
        Ok(())
    }
