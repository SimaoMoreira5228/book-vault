...
impl AssetService {
	...
	pub async fn get_image_data(storage: &dyn crate::storage::StorageProvider, asset_id: Uuid) -> Result<Vec<u8>, AppError> {
		let key = format!("asset/{}", asset_id);
		storage.get(&key).await
	}

	pub async fn get_asset(db: &sea_orm::DatabaseConnection, asset_id: Uuid) -> Result<assets::Model, AppError> {
	...
	pub async fn list_assets(db: &sea_orm::DatabaseConnection, book_id: Uuid) -> Result<Vec<assets::Model>, AppError> {
