	...
	match block {
		...
		Block::Table { headers, rows } => {
			...
			let body_rows: String = rows
				.iter()
				.map(|row| {
					let cells: String = row.iter().map(|c| format!("<td>{}</td>", render_spans(&c.spans))).collect();
					format!("<tr>{cells}</tr>")
				})
				.collect();
...
impl Exporter for EpubExporter {
	async fn export(&self, ir: &BookIr) -> Result<Vec<u8>, crate::AppError> {
		let zip = ZipLibrary::new().map_err(|e| crate::AppError::Internal(format!("Failed to init ZIP: {e}")))?;
		let mut builder =
			EpubBuilder::new(zip).map_err(|e| crate::AppError::Internal(format!("Failed to create EPUB: {e}")))?;

		let title = ir.spine.first().and_then(|s| s.title.as_deref()).unwrap_or("Untitled");
		...
		builder
			.stylesheet(Cursor::new(CSS.as_bytes()))
