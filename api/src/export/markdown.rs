...
fn render_spans(spans: &[Span]) -> String {
    ...
    for s in spans {
        ...
        if s.marks & 8 != 0 { text = format!("~~{text}~~"); }
        if s.marks & 2 != 0 { text = format!("*{text}*"); }
        if s.marks & 1 != 0 { text = format!("**{text}**"); }
...
fn render_block(block: &Block) -> String {
    match block {
        ...
        Block::Table { headers, rows } => {
            let header: String = headers.iter()
                .map(|h| format!("| {}", render_spans(&h.spans)))
                .collect::<Vec<_>>().join("") + "|";
            let sep: String = headers.iter()
                .map(|_| "| --- ")
                .collect::<Vec<_>>().join("") + "|";
