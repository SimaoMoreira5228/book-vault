...
fn collect_lines(ir: &BookIr) -> Vec<TextLine> {
    ...
    for section in &ir.spine {
        if let Some(title) = &section.title {
            lines.push(TextLine { text: title.clone(), size: HEADING_SIZE });
        }
        for block in &section.blocks {
            ...
            for (j, l) in block_lines.iter().enumerate() {
                if is_heading && j == 0 {
                    lines.push(TextLine { text: l.clone(), size: HEADING_SIZE });
                } else {
                    lines.push(TextLine { text: l.clone(), size: FONT_SIZE });
                }
            }
            lines.push(TextLine { text: String::new(), size: FONT_SIZE });
        }
    }
    lines
}
