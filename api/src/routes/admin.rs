...
async fn list_jobs(State(state): State<SharedState>, auth: AuthenticatedUser) -> Result<Json<Vec<JobResponse>>, AppError> {
...
async fn list_users(
	...
	let users = Users::find().order_by_desc(users::Column::CreatedAt).all(&state.db).await?;
