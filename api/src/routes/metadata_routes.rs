...
async fn search_candidates(
	...
	let results = state.metadata_service.search_candidates(&state.db, book_id, &query).await?;
	Ok(Json(results))
}
...
async fn refresh_metadata_handler(
	...
	verify_book_access(&state, &auth, book_id).await?;
	let result = state.metadata_service.refresh_metadata(&state.db, book_id).await?;
	Ok(Json(result))
}
...
async fn lock_field(
	...
	verify_book_access(&state, &auth, book_id).await?;
	let field =
		MetadataField::from_str(&field_str).map_err(|_| AppError::BadRequest(format!("Invalid field: {field_str}")))?;
	let result = state.metadata_service.lock_field(&state.db, book_id, field).await?;
	Ok(Json(result))
}
...
async fn unlock_field(
	...
	let field =
		MetadataField::from_str(&field_str).map_err(|_| AppError::BadRequest(format!("Invalid field: {field_str}")))?;
	let result = state.metadata_service.unlock_field(&state.db, book_id, field).await?;
	Ok(Json(result))
}

async fn verify_book_access(state: &SharedState, auth: &AuthenticatedUser, book_id: Uuid) -> Result<(), AppError> {
