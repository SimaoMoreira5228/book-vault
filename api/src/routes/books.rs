...
pub async fn get_user_library_ids(db: &sea_orm::DatabaseConnection, user_id: Uuid) -> Result<Vec<Uuid>, AppError> {
...
async fn update_book(
	...
	if let Some(v) = req.isbn {
		active.isbn = Set(Some(v));
	}
	if let Some(v) = req.language {
		active.language = Set(Some(v));
	}
	if let Some(v) = req.publisher {
		active.publisher = Set(Some(v));
	}
	if let Some(v) = req.series {
		active.series = Set(Some(v));
	}
	if let Some(v) = req.series_index {
		active.series_index = Set(Some(v));
	}
	if let Some(v) = req.page_count {
		active.page_count = Set(Some(v));
	}
	if let Some(v) = req.read_status {
		active.read_status = Set(v);
	}
