...
use crate::db::entities::{books, job_queue};
...
impl JobWorker {
	...
	async fn process_one(state: &SharedState) -> Result<(), AppError> {
		...
		if let Some(job) = job {
			...
			notify(format!(
				r#"{{"job_id":"{}","status":"processing","kind":"{}"}}"#,
				job.id, kind
			));
			...
			match result {
				...
				Err(e) => {
					...
					if new_count >= job.max_retries {
						...
						notify(format!(
							r#"{{"job_id":"{}","status":"dead_letter","kind":"{}"}}"#,
							job.id, kind
						));
	...
	async fn dispatch(state: &SharedState, kind: &str, job: &job_queue::Model) -> Result<(), AppError> {
