...
async fn register_create_book_then_annotate() {
    ...
    let (status, export_result) = app.raw_get(&format!("/api/v1/books/{}/export?format=markdown", book_id)).await;
    assert!(status == 200 || status == 404 || status == 500, "export should not crash: {status}");
    if status == 200 {
        let _ = export_result;
    }
}
...
async fn full_lifecycle() {
    ...
    // Update book
    let (status, updated) = app.raw_put(
