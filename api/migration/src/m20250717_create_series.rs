...
impl MigrationTrait for Migration {
	async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
		manager.create_table(
			Table::create()
				.table(Series::Table).if_not_exists()
				...
				.foreign_key(ForeignKey::create().name("fk_series_lib").from(Series::Table, Series::LibraryId).to(Libraries::Table, Libraries::Id).on_delete(ForeignKeyAction::Cascade))
				.to_owned(),
		).await
	}
...
#[derive(DeriveIden)]
pub enum Series { Table, Id, LibraryId, Name, Description, CreatedAt, UpdatedAt }
